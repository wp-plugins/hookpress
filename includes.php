...
<td><input type='radio' id='action' class='type' name='newtype' checked='checked'> <?php _e("action","hookpress");?></input> 
<input type='radio' id='filter' class='type' name='newtype'> <?php _e("filter","hookpress");?></input></td></tr>
<tr>
<td><label style='font-weight: bold' for='edithook' class='action_or_filter'>
<?php
if ($desc['type'] == 'action')
	echo 'Action:';
if ($desc['type'] == 'filter')
...
<td><select name='edithook' id='edithook' class='hook'>
    <?php  	
      sort($hooks);
...
<br/>
<span class='filtermessage'><small><?php _e('The first argument of a filter must always be sent and should be returned by the webhook, with modification.','hookpress');?></small></span>
</td>
<td>
<select style='vertical-align: top' name='editfields' id='editfields' class='fields' multiple='multiple' size='8'>
<?php
	global $wpdb, $hookpress_actions, $hookpress_filters;
  if ($desc['type'] == 'action')
...
?></select></td></tr>
<tr><td><label style='font-weight: bold' for='editurl'><?php _e("URL",'hookpress');?>: </label></td>
<td><input name='editurl' id='editurl' class='url' size='40' value="<?php echo $desc['url']; ?>"></input></td></tr>
</table>
<?php  echo $nonce_submit; ?>
  <center><span id='editindicator' class='indicator'></span><br/>
  <input type='button' class='button' id='editsubmit' class='submit' value='<?php _e('Save webhook','hookpress');?>'/>
  <input type='button' class='button' id='editcancel' class='cancel' value='<?php _e('Cancel');?>'/></center>
